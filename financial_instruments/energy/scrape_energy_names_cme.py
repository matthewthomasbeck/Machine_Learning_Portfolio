...
def find_cme_subpages(): # function to find subpages on CME website
    ...
    CMEWebpage = requests.get( # connect to CME exchange with fake user agent
        ...
        'https://www.cmegroup.com/markets/energy.html#products',
        timeout=10,
    ...
    if CMEWebpage.status_code == 200: # if successfully connected to CME...
        ...
        for anchorTag in anchorTags: # loop through every anchor tag found
            ...
            if CMESubpage.startswith(f'/trading/energy/') or CMESubpage.startswith(f'/markets/energy/'):
...
def find_cme_commodity_pages(): # function to get a list of commodity page links
    ...
    for CMESubpage in CMESubpages: # loop through every subpage found
        ...
        try: # attempt to open the dynamic webpage with webdriver...
            ...
            oilListElements = CMESubpageParser.find_all( # find elements that contain anchor tags for products

                ['td', 'th'],
                class_='cmeTableLeft'
            )

            if not oilListElements: # if a page is empty...

                carbAnchorTagAttributes = { # set attributes to look for anchor tag

                    'class': 'btn link-bold crmb-3',
                    'data-target-description': 'none',
                    'rel': 'nofollow'
                }

                carbAnchorTag = CMESubpageParser.find( # find anchor element that holds next link

                    'a',
                    attrs=carbAnchorTagAttributes
                )

                if carbAnchorTag: # if anchor tag is found...

                    # add link to list of commodity pages (used for carbon emissions exception)
                    CMESubpages.append('https://www.cmegroup.com' + carbAnchorTag.get('href'))

            else: # if a page contains elements...

                for oilListElement in oilListElements: # loop through every listing element

                    oilAnchorTag = oilListElement.find( # find all anchor tags within elements that may contain a link

                        'a',
                    )

                    if oilAnchorTag: # if anchor tag containing a link found...

                        CMECommodityPages.append( # save link to commodity pages list with specs

                            'https://www.cmegroup.com' + oilAnchorTag.get('href')
                        )
...
def find_cme_commodity_specs(): # function to get a list of all agricultural commodity specs
    ...
    for CMECommodityPage in CMECommodityPages: # loop through every commodity page name
        ...
        try: # attempt to open the dynamic webpage with webdriver...
            ...
            anchor = CMECommodityPageParser.find('div', attrs=specsAttributes).find('a')

            CMECommoditySpecs.append(anchor.get('href')) # save found link to list of spec pages
    ...
    with open( # save globex tickers to globex tickers file
            ...
            './financialInstrument_data/energy_data/energy_data-current_globex_tickers-CME.txt',
            'w'
...
def find_cme_energy_names(): # function to get a list of all agricultural commodity names
    ...
    for CMECommoditySpec in CMECommoditySpecs: # loop through every commodity page name
        ...
        try: # attempt to open the dynamic webpage with webdriver...
            ...
            # print webdriver failed to work error
            print(f'Failed to connect to CME subpage. Returned with error: "{e}"\n')
    ...
    with open( # save commodity names to commodity names file
        ...
        './financialInstrument_data/energy_data/energy_data-clearing_tickers-CME.txt',
        'w'
